...
Every serious project needs a scripted way to drive the real app and prove behavior: launch it, exercise a feature the way a user would, and capture evidence. This skill generates that as a project-local skill tailored to the repo. Use `.cursor/skills/` on Cursor, `.agents/skills/` on Codex, `.claude/skills/` on Claude Code, or the configured project skill directory on OpenCode. You write the output for the next agent, not for a human: it will be read cold, mid-task, by an agent that has never seen the app.
