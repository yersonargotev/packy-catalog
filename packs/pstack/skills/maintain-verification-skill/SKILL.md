...
0. **Locate the target.** Find the verification skill in the active host's project skill directory: `.cursor/skills/`, `.agents/skills/`, `.claude/skills/`, or the configured OpenCode directory. Its body has launch/drive sections and a feature map. Several candidates → ask which one; none → stop and point at `create-verification-skill` instead of inventing a target.
