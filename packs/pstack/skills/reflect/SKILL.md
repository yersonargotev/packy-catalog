...
The parent uses the active host's workspace transcript when one is exposed.
On Cursor, the system prompt names its `agent-transcripts/` directory. Do not
search other projects' private chats. On another host, use its documented
active-session export or a concise digest of the conversation.
