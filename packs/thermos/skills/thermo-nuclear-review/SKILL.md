...
description: Audit a diff for correctness, security, developer-experience regressions, and feature-gate leaks.
...
# Final Response
IF you have medium-to-high priority / risk findings, and there is a PR for this branch, then read the PR/MR discussion using an available authorized connector or authenticated `gh`/`glab` CLI to see if there are comments from BugBot or others present. If discussion access is unavailable, state that limitation and return the independently verified findings. Reading discussion does not authorize posting comments.
